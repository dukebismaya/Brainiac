(function(){
  ...
  (function initTheme(){
    ...
    if (localStorage.theme === 'light' || (!('theme' in localStorage) && prefersLight)) {
  ...
  // ✅ helper to reapply vibrant styles after theme switch
  function refreshThemeUI(){
    document.querySelectorAll('.stylish-btn').forEach(btn=>{
      btn.classList.remove('theme-refresh');
      void btn.offsetWidth; // reflow
      btn.classList.add('theme-refresh');
    });
  }
  ...
  if (themeToggle) {
    themeToggle.addEventListener('click', () => {
      ...
      refreshThemeUI(); // ✅ reapply border glow
    });
  }
